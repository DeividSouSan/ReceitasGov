...
from bots.get_public_revenue_bot import RevenueBot
...
def open_file():
    ...
    open_button = tk.Button(open_file_window, text="Abrir Arquivo", command=open_file)
    open_button.pack(pady=10)

    open_button = tk.Button(
        open_file_window, text="Sair", command=open_file_window.destroy
    )
    open_button.pack(pady=10)
...
def configs_action():
    ...
    # Criar botão "Salvar"
    salvar_button = tk.Button(config_window, text="Salvar", command=save)
    salvar_button.pack(side=tk.LEFT, padx=10, pady=10)

    # Criar botão "Sair"
    quit_button = tk.Button(config_window, text="Sair", command=config_window.destroy)
    quit_button.pack(side=tk.RIGHT, padx=10, pady=10)


def close():
    logging.info(
        f"Finalizando o programa: {datetime.now().strftime('%d/%m/%Y, %H:%M:%S')}..."
    )
    root.destroy()
...
title_label = tk.Label(
    root, text=config["Bot"]["BOT_NAME"], font=("Helvetica", 16, "bold")
)
...
status_label = tk.Label(
    root, text=f"A página está: {status_label_text}", font=("Helvetica", 10)
)
